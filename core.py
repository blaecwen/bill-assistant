...
async def process_message(
    ...
    if request is not None:
        ...
        if photo_store.get_photo(chat_id) is None:
            return BillResponse(text=_NO_PHOTO_MSG, needs_input=True)

        # B3: Photo is stale, not yet asked
        if photo_store.is_photo_stale(chat_id):
            text_request = request if isinstance(request, str) else None
            if text_request:
                photo_store.set_pending_request(chat_id, text_request)
...
async def _call_and_respond(
    ...
    try:
        ...
        return BillResponse(text=_LLM_ERROR_MSG)
