...
class PhotoStore:
    ...
    def store_photo(self, session_id: str, data: bytes) -> None:
    ...
    def get_photo(self, session_id: str) -> Optional[StoredPhoto]:
        return self._get_state(session_id).photo

    def _photo_age_minutes(self, session_id: str) -> Optional[float]:
        photo = self._get_state(session_id).photo
        if photo is None:
            return None
        delta = datetime.now(timezone.utc) - photo.stored_at
        return delta.total_seconds() / 60

    def is_photo_fresh(self, session_id: str) -> bool:
        age = self._photo_age_minutes(session_id)
        return age is not None and age < self._ttl_minutes

    def is_photo_stale(self, session_id: str) -> bool:
        age = self._photo_age_minutes(session_id)
        if age is None:
            return False
    ...
    def set_pending_request(self, session_id: str, request: str) -> None:
        self._get_state(session_id).pending_request = request

    def get_pending_request(self, session_id: str) -> Optional[str]:
        return self._get_state(session_id).pending_request

    def set_awaiting_stale_confirmation(self, session_id: str, val: bool) -> None:
        self._get_state(session_id).awaiting_stale_confirmation = val

    def is_awaiting_stale_confirmation(self, session_id: str) -> bool:
        return self._get_state(session_id).awaiting_stale_confirmation
